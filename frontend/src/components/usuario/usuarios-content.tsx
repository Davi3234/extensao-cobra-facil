  ...
  const inativar = (id: number) => {
    startTransition(async () => {
      try {
        ...
        if (response.ok) {
          notify({ type: 'success', message: 'Usu√°rio inativado com sucesso' })
        } else {
          notify({ type: 'error', message: response.error || 'Erro ao inativar usu√°rio' })
        }
      } catch (error) {
        notify({ type: 'error', message: 'Erro ao inativar usu√°rio' })
      }
    })
  }
    ...
    <div className='grid grid-cols-2 gap-4'>
      ...
      <div className='bg-white p-4 rounded shadow'>
        ...
        <div className='space-y-2'>
            ...
            <div key={usuario.id}>
              <div className='flex items-center justify-between p-2'>
                <div>
                  ...
                  <div className='text-xs text-gray-500'>{usuario.email} ‚ˇ¢ {usuario.telefone}</div>
                </div>
                ...
                <div className='flex gap-1'>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Deseja inativar o usu√°rio?</AlertDialogTitle>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
          ...
          {usuarios.length === 0 && <div className='text-sm text-gray-500'>Nenhum usu√°rio cadastrado.</div>}
        </div>
      </div>
    </div>
  )
